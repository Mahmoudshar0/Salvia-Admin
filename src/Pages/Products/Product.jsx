...
const ProductsData = [
  { id: 1, name: "ANISE SEEDS", description: "PIMPINELLA ANISUM", image: anise },
  { id: 2, name: "ARTICHOKE", description: "CYNARA SCOLYMUS", image: artichoke },
  { id: 3, name: "BASIL", description: "OCIMUM BASILICUM", image: basil },
  { id: 4, name: "CARAWAY SEEDS", description: "MATRICARIA CHAMOMILLA", image: caraway },
];
...
export default function Product() {
  ...
  const handleEditProduct = (product) => {
    navigate(`/products/edit/${product.id}`, { state: { productData: product } });
  };
    ...
    <div className="flex">
      ...
      <div className="flex-1 ml-[260px] bg-gray-100 min-h-screen p-14">
        <div className="flex justify-between items-center mb-8">
          <div className="w-1/2">
            <h2 className="text-2xl font-semibold text-gray-800 mb-3">Page title</h2>
            <input
              type="text"
              value="Products"
              readOnly
          ...
          {filteredProducts.map((product) => (
            ...
            >
              ...
              <div className="flex justify-center items-center bg-white p-5 h-auto mt-4">
                <img src={product.image} alt={product.name} className="h-46 w-auto object-contain" />
              </div>

              <div className="w-[90%] bg-[#4E6347] text-white rounded-xl py-4 px-2 text-center mb-5 mt-2 shadow-sm group-hover:bg-[#58754E]">
                <h3 className="font-bold text-lg uppercase">{product.name}</h3>
                <p className="font-light italic mt-1 text-sm">{product.description}</p>
              </div>
            </div>
          ))}
        </div>
